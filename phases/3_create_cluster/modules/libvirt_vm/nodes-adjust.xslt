<?xml version="1.0" ?>
<!-- 
  This file modifies the XML that libvirt uses to define VMs. 
  These modifications are necessary because OpenTofu's libvirt module 
  does not allow us to make these changes directly:

  1. Change the type of the CD-ROM to be SATA instead of IDE.
  2. Assign serial numbers to extra disks, creating identifiable 
     symlinks in /dev/disk/by-id/ so that subsequent phases 
     can reliably reference target disks.
-->
...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  ...
  <!-- Store the VM Name for use in disk serials -->
  ...
  <xsl:template match="/domain/devices/disk[source and target][not(target/@dev='vda')]">
    <xsl:copy>
      <xsl:apply-templates select="@*|node()"/>
      <serial>
        <xsl:value-of select="concat($vm_name, '.', substring(target/@dev, 3, 1))"/>
      </serial>
    </xsl:copy>
  </xsl:template>
