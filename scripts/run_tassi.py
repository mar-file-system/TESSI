...
def create_gold(conn, bootstrap_vm, hname, inventory_file, playbook_file, group, verbosity, ansible_log_prefix=None, dnfplaybook=None, dnfplaybooktags=None):
    ...
    if dnfplaybook:
        logger.debug(f"Running ansible playbook {dnfplaybook} on {group}:{hname} with inventory {inventory_file}") 
        run_playbook(hname, inventory_file, dnfplaybook, group, verbosity, f"{ansible_log_prefix}.{hname}", None, dnfplaybooktags)
...
def run_playbook(hname, inventory_file, playbook_file, group, verbosity, output_prefix=None, extravars=None, tags=None):
    logger = logging.getLogger(__name__)
    ...
    logger.debug(f"Running with kwargs {kwargs}")
...
def make_gold_vms(conn,bootstrap_vm,images,system,inventory,inventory_file,playbook_file,rebuild_vms,rebuild_golds,verbosity, ansible_log_prefix, dnfplaybook, dnfplaybooktags):
    logger = logging.getLogger(__name__)
    ...
    for group,gold in golds.items(): 
        ...
        if gold['image'] and not rebuild_golds:
            ...
            kernel_version = create_gold(conn, bootstrap_vm, gold['hname'], inventory_file, playbook_file, group, verbosity, ansible_log_prefix, dnfplaybook, dnfplaybooktags)
...
def is_server_running(url):
    ...
    try:
        ...
        # If the server responds with a status code, return True
        ...
        # Return False if there's any connection error
        logger.debug(f"RequestException from {url}: {e}")
        return False

def confirm_input_arguments(system, vm_dir, inventory, inventory_file, dnfmirror, dnfplaybook):

    check_images_directory(system,vm_dir)

    playbook_paths = [get_inventory_value(inventory, f"all.vars.ansible_playbooks.{playbook}", required=True) for playbook in ['install', 'config', 'test']]
    playbook_paths += [dnfplaybook] if dnfplaybook else []
    for playbook_path in playbook_paths: 
        if not os.path.exists(playbook_path):
            Fatal(f"Ansible {playbook} playbook at specified path {playbook_path} does not exist")
        check_playbook(inventory_file, playbook_path)

    test_path = get_inventory_value(inventory, f"all.vars.test_script.path", required=True)
    if not os.path.exists(test_path):
        Fatal(f"Test script at specified path {test_path} does not exist")

    if dnfmirror and not is_server_running(dnfmirror):
        Fatal(f"There is not a server serving {dnfmirror}")
...
def main():
    ...
    dnfmirror       = get_inventory_value(inventory, 'all.vars.dnfmirror.mirror_url', required=False)
    dnfplaybook     = get_inventory_value(inventory, 'all.vars.dnfmirror.config_playbook', required=False)
    ...
    confirm_input_arguments(system, vm_dir, inventory, args.inventory_file, dnfmirror, dnfplaybook) 
    ...
    with LibvirtConnection() as conn:
        ...
        (gold_vms['servers'], gold_vms['clients']) = make_gold_vms(
            ...
            dnfplaybook = dnfplaybook,
            dnfplaybooktags = None
            )
